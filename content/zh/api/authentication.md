...
要访问 {brand} API 的任何端点，您需要对请求进行认证。{brand} 提供两种类型的令牌进行认证：`api_key` 和 `secret_key`。这些令牌用于加密每个请求中的参数。

加密的参数会通过我们的系统使用记录中的相同令牌进行解密和验证。一旦验证成功，请求就会被处理，并返回响应。

---

## 安全

我们有自己的算法来生成请求以命中我们的端点。该算法参照 [安全前提条件](/docs/pre-requirements#security) ，需要为每个交易生成密钥。
